...
static const PoeInfo l_poeInfo[] = {
    // Dungeons
    {"D_MN06", 0x19, -1, 0x15},
    {"D_MN06", 0x18, -1, 0x15},
    {"D_MN07", 0x54, -1, 0x16},
...
void dMenuMapCommon_c::getDmapPoeCount(const char* stageName, int& nowCount, int& totalCount) {
    nowCount = 0;
    totalCount = 0;

    for (int i = 0; i < (sizeof(l_poeInfo) / sizeof(l_poeInfo[0])); i++) {
        const PoeInfo& info = l_poeInfo[i];
        if (strncmp(stageName, info.stagename, sizeof(char [6])) == 0) {
            if (dStage_stagInfo_GetSaveTbl(dComIfGp_getStageStagInfo()) == info.save_id) {
                nowCount += dComIfGs_isSwitch(info.switch_no, -1);
            } else {
                nowCount += dComIfGs_isSaveSwitch(info.save_id, info.switch_no);
            }
            totalCount++;
        }
    }
