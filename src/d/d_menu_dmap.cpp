...
{
    static u64 const cont_at[5] = {MULTI_CHAR('cont_at'),
                                   MULTI_CHAR('cont_at1'),
                                   MULTI_CHAR('cont_at2'),
                                   MULTI_CHAR('cont_at3'),
                                   MULTI_CHAR('cont_at4')};
    static u64 const cont_bt[5] = {MULTI_CHAR('cont_bt'),
                                   MULTI_CHAR('cont_bt1'),
                                   MULTI_CHAR('cont_bt2'),
                                   MULTI_CHAR('cont_bt3'),
                                   MULTI_CHAR('cont_bt4')};
    static u64 const font_at[5] = {MULTI_CHAR('font_at'),
                                   MULTI_CHAR('font_at1'),
                                   MULTI_CHAR('font_at2'),
                                   MULTI_CHAR('font_at3'),
                                   MULTI_CHAR('font_at4')};
    static u64 const font_bt[5] = {MULTI_CHAR('font_bt'),
                                   MULTI_CHAR('font_bt1'),
                                   MULTI_CHAR('font_bt2'),
                                   MULTI_CHAR('font_bt3'),
                                   MULTI_CHAR('font_bt4')};
...
void dMenu_DmapBg_c::baseScreenInit()
{
    ...
    fg = mFloorScreen->setPriority("zelda_dungeon_map_floor_parts.blo", 0x20000, dComIfGp_getDmapResArchive());
...
void dMenu_DmapBg_c::draw()
{
    ...
    if (mpBackTexture != NULL)
    {
        ...
        grafContext->scissor(((local_200.x - mDoGph_gInf_c::getMinXF()) / dVar21),
                             ((local_200.y / dVar16) / dVar16),
                             ((local_20c.x - local_200.x) / dVar21),
                             2.0f + (local_20c.y - local_200.y));
        grafContext->setScissor();
        ...
        mpBackTexture->draw(local_28c,
                            field_0xd94 + mpBackTexture->getBounds().i.y,
                            mpBackTexture->getWidth(),
...
dMenu_Dmap_c::dMenu_Dmap_c(JKRExpHeap* param_1, STControl* param_2, CSTControl* param_3, u8 param_4, u8 param_5)
{
...
{
    static u64 const floor_tag[8] = {MULTI_CHAR('floor7_n'),
                                     MULTI_CHAR('floor0_n'),
                                     MULTI_CHAR('floor1_n'),
                                     MULTI_CHAR('floor2_n'),
                                     MULTI_CHAR('floor3_n'),
                                     ...
                                     MULTI_CHAR('floor5_n'),
                                     MULTI_CHAR('floor6_n')};
    static u64 const icon_tag[8] = {MULTI_CHAR('ico_set7'),
                                    MULTI_CHAR('ico_set0'),
                                    MULTI_CHAR('ico_set1'),
                                    MULTI_CHAR('ico_set2'),
                                    MULTI_CHAR('ico_set3'),
                                    ...
                                    MULTI_CHAR('ico_set5'),
                                    MULTI_CHAR('ico_set6')};
    static u64 const boss_tag[8] = {MULTI_CHAR('ic_st_b7'),
                                    MULTI_CHAR('ic_st_b0'),
                                    MULTI_CHAR('ic_st_b1'),
                                    MULTI_CHAR('ic_st_b2'),
                                    MULTI_CHAR('ic_st_b3'),
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (i == 0)
        {
            mpDrawBg->mFloorScreen->search('wolf')->move((f32)mpDrawBg->mFloorScreen->search('rink')->getBounds().i.x,
                                                         (f32)mpDrawBg->mFloorScreen->search('rink')->getBounds().i.y);
    ...
    mpDrawBg->mpDrawCursor->setPos(local_b0.x + field_0x104,
                                   local_b0.y,
                                   mSelFloor[getDefaultCurFloorPos()]->getPanePtr(),
                                   true);
    ...
    if (dStage_stagInfo_GetSaveTbl(dComIfGp_getStageStagInfo()) == dStage_SaveTbl_LV2)
    {
        ...
        {
            ResTIMG* tex =
                (ResTIMG*)dComIfGp_getMain2DArchive()->getResource('TIMG', dMeter2Info_getNumberTextureName(key_num));
    ...
    else if (dStage_stagInfo_GetSaveTbl(dComIfGp_getStageStagInfo()) == dStage_SaveTbl_LV5)
    {
        ...
        switch (itemNo)
        {
            ...
            case dItemNo_TASTE_e:
                dMeter2Info_readItemTexture(itemNo,
                                            mItemTexBuf,
                                            (J2DPicture*)mpDrawBg->mBaseScreen->search(MULTI_CHAR('lv5_boss')),
                                            NULL,
                                            NULL,
                                            NULL,
                                            NULL,
    ...
    if (bVar6 == true)
    {
        field_0x94->getPanePtr()->translate(field_0x7c[field_0x177]->getPanePtr()->getTranslateX(),
                                            field_0x7c[field_0x177]->getPanePtr()->getTranslateY());
    }
...
dMenu_Dmap_c::~dMenu_Dmap_c()
{
    ...
    OS_REPORT("mpDrawBg free ==================> %d bytes use,  %d bytes remain\n",
              field_0xe8->getTotalFreeSize() - iVar4,
              field_0xe8->getTotalFreeSize());
    ...
    OS_REPORT("MenuDmap free ==================> %d bytes use,  %d bytes remain\n",
              field_0xe8->getTotalFreeSize() - iVar2,
              field_0xe8->getTotalFreeSize());
...
void dMenu_Dmap_c::drawFloorScreenBack(J2DScreen* i_screen, f32 i_transX, f32 i_transY, J2DOrthoGraph* i_ortho)
{
...
void dMenu_Dmap_c::drawFloorScreenTop(J2DScreen* i_screen, f32 i_transX, f32 i_transY, J2DOrthoGraph* i_ortho)
{
    for (int i = 0; i < getFloorAll(); i++)
    {
        if (i == getCurFloorPos() && 0.0f == field_0x114[i])
...
void dMenu_Dmap_c::_create()
{
    ...
    OS_REPORT("mpDrawBg create==================> %d bytes use,  %d bytes remain\n",
              sp24 - field_0xe8->getTotalFreeSize(),
              field_0xe8->getTotalFreeSize());
    ...
    if (dMeter2Info_getMapStatus() == 6)
    {
        ...
        mMapCtrl->_create(mpDrawBg->getMapWidth(),
                          mpDrawBg->getMapHeight(),
                          mpDrawBg->getMapWidth(),
                          mpDrawBg->getMapHeight(),
                          sp8,
                          mpBinData);
        m_process = 4;
...
void dMenu_Dmap_c::mapControl()
{
    ...
    f32 sp28 = g_fmapHIO.mScrollSpeedSlowBound < g_fmapHIO.mScrollSpeedFastBound ? g_fmapHIO.mScrollSpeedSlowBound
                                                                                 : g_fmapHIO.mScrollSpeedFastBound;
    f32 sp24 = g_fmapHIO.mScrollSpeedSlowBound > g_fmapHIO.mScrollSpeedFastBound ? g_fmapHIO.mScrollSpeedSlowBound
                                                                                 : g_fmapHIO.mScrollSpeedFastBound;
    ...
    if (mMapCtrl != NULL)
    {
        ...
        if (mpDrawBg != NULL)
        {
            ...
            if (mpDrawBg->mpBackTexture != NULL)
            {
                ...
                Vec spC;
                spC.x = mpDrawBg->mMapScreen[0]->search(MULTI_CHAR('center_n'))->getGlbBounds().i.x +
                        (mpDrawBg->mMapScreen[0]->search(MULTI_CHAR('center_n'))->getWidth() / 2);
                spC.y = mpDrawBg->mMapScreen[0]->search(MULTI_CHAR('center_n'))->getGlbBounds().i.y +
                        (mpDrawBg->mMapScreen[0]->search(MULTI_CHAR('center_n'))->getHeight() / 2);

                CPaneMgr sp70;
                //!@bug It's unclear what this is supposed to be, but a stack pointer being converted to a bool is probably not
                //! intended.
                u8 sp40[0x30];
...
u8 dMenu_Dmap_c::getNextStatus()
{
    if (POINTER_OPT != 0)
    {
        if ((dMw_LEFT_TRIGGER() || (dMw_B_TRIGGER() && (!getZoomState() || (m_process == 0 && m_itemSubProcess == 0)))) &&
            !isKeyCheck())
        {
            return 1;
        }
...
void dMenu_Dmap_c::zoomWait_proc()
{
    if (m_process == 1)
    {
        if (mDoCPd_c::getTrigA(PAD_1) &&
            (((POINTER_OPT == 1 && mpDrawBg->field_0xdd3 != 0xFF) || POINTER_OPT == 0) && !dMeter2Info_isTouchKeyCheck(0xC)))
        {
