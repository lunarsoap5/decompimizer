...
dMeter2Draw_c::dMeter2Draw_c(JKRExpHeap* mp_heap)
{
    ...
    void* stg_msg_res = JKRGetTypeResource('ROOT', bmg_filename[dMsgObject_getGroupID()], dComIfGp_getMsgDtArchive(1));
    ...
    fg = mpKanteraScreen->setPriority("zelda_game_image_kantera.blo", 0x20000, dComIfGp_getMain2DArchive());
    JUT_ASSERT(0, fg != false);
    ...
    static u64 const a_tag[] = {MULTI_CHAR('cont_at1'),
                                MULTI_CHAR('cont_at2'),
                                MULTI_CHAR('cont_at3'),
                                MULTI_CHAR('cont_at4'),
                                MULTI_CHAR('cont_at')};
    static u64 const b_tag[] = {MULTI_CHAR('cont_bt1'),
                                MULTI_CHAR('cont_bt2'),
                                MULTI_CHAR('cont_bt3'),
                                MULTI_CHAR('cont_bt4'),
                                MULTI_CHAR('cont_bt')};
    static u64 const z_tag[] = {MULTI_CHAR('cont_zt1'),
                                MULTI_CHAR('cont_zt2'),
                                MULTI_CHAR('cont_zt3'),
                                MULTI_CHAR('cont_zt4'),
                                MULTI_CHAR('cont_rt')};
    static u64 const x_tag[] = {MULTI_CHAR('cont_xt1'),
                                MULTI_CHAR('cont_xt2'),
                                MULTI_CHAR('cont_xt3'),
                                MULTI_CHAR('cont_xt4'),
                                MULTI_CHAR('cont_xt5')};
    static u64 const y_tag[] = {MULTI_CHAR('cont_yt1'),
                                MULTI_CHAR('cont_yt2'),
                                MULTI_CHAR('cont_yt3'),
                                MULTI_CHAR('cont_yt4'),
                                MULTI_CHAR('cont_yt5')};
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        static_cast<J2DTextBox*>(mpAText[i]->getPanePtr())->setCharSpace(g_drawHIO.mButtonATextSpacing);
...
void dMeter2Draw_c::exec(u32 i_status)
{
    ...
    {
        if (mButtonsPosX != g_drawHIO.mRingHUDButtonsPosX || mButtonsPosY != g_drawHIO.mRingHUDButtonsPosY)
        {
    ...
    {
        if (mButtonsPosX != g_drawHIO.mMainHUDButtonsPosX || mButtonsPosY != g_drawHIO.mMainHUDButtonsPosY)
        {
...
void dMeter2Draw_c::draw()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        if (mpItemXY[i] != NULL)
        {
            for (int j = 0; j < 3; j++)
            {
                ...
                mpItemNumTex[i][j]->draw(mItemParams[i].num_pos_x + (((vtx0.x + vtx3.x) * 0.5f) + (temp_f30 * j)),
                                         mItemParams[i].num_pos_y + (((vtx0.y + vtx3.y) * 0.5f) + mpItemXY[i]->getSizeY()),
                                         temp_f30,
                                         temp_f30,
                                         false,
                                         false,
    ...
    if (!dComIfGp_isPauseFlag() && mpButtonParent->getAlphaRate() != 0.0f)
    {
        if (field_0x608 > 0.0f)
        {
            drawPikari(mpBTextA,
                       &field_0x608,
                       g_drawHIO.mButtonAPikariScale,
                       g_drawHIO.mButtonAPikariFrontOuter,
                       g_drawHIO.mButtonAPikariFrontInner,
                       g_drawHIO.mButtonAPikariBackOuter,
                       g_drawHIO.mButtonAPikariBackInner,
                       g_drawHIO.mButtonAPikariAnimSpeed,
                       field_0x759);
        }
        ...
        if (field_0x60c > 0.0f)
        {
            drawPikari(mpBTextB,
                       &field_0x60c,
                       g_drawHIO.mButtonBPikariScale,
                       g_drawHIO.mButtonBPikariFrontOuter,
                       g_drawHIO.mButtonBPikariFrontInner,
                       g_drawHIO.mButtonBPikariBackOuter,
                       g_drawHIO.mButtonBPikariBackInner,
                       g_drawHIO.mButtonBPikariAnimSpeed,
                       field_0x75a);
        }
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            {
                drawPikari(mpBTextXY[i],
                           &field_0x620[i],
                           g_drawHIO.mButtonXYPikariScale,
                           g_drawHIO.mButtonXYPikariFrontOuter,
                           g_drawHIO.mButtonXYPikariFrontInner,
                           g_drawHIO.mButtonXYPikariBackOuter,
                           g_drawHIO.mButtonXYPikariBackInner,
                           g_drawHIO.mButtonXYPikariAnimSpeed,
                           field_0x75c[i]);
            }
        }
    }
    ...
    if (mpLightDropParent->getAlphaRate() != 0.0f)
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            {
                drawPikari(mpSIParts[i][1],
                           &g_drawHIO.mLightDrop.mPikariLoopBackStopFrame,
                           ...
                           g_drawHIO.mLightDrop.mPikariLoopBackInner[1],
                           0.0f,
                           3);
                drawPikari(mpSIParts[i][1],
                           &field_0x66c[i],
                           g_drawHIO.mLightDrop.mPikariLoopScale,
                           ...
                           g_drawHIO.mLightDrop.mPikariLoopAnimSpeed,
                           3);
            }

            if (g_drawHIO.mLightDrop.mAnimDebug)
            {
                if (dComIfGp_getNeedLightDropNum() != dComIfGs_getLightDropNum(dComIfGp_getStartStageDarkArea()))
                {
                    field_0x66c[i] = 0.0f;
                }
            }
        ...
        for (int i = 0; i < 16; i++)
        {
            if (field_0x62c[i] > 0.0f)
            {
                drawPikari(mpSIParts[i][1],
                           ...
                           g_drawHIO.mLightDrop.mDropPikariBackInner,
                           var_f29,
                           field_0x75f);
            }
        }
    ...
    if (field_0x738 > 0.0f)
    {
        drawPikari(mpButtonMidona,
                   &field_0x738,
                   g_drawHIO.mMidnaIconPikariScale,
                   g_drawHIO.mMidnaIconPikariFrontOuter,
                   g_drawHIO.mMidnaIconPikariFrontInner,
                   g_drawHIO.mMidnaIconPikariBackOuter,
                   g_drawHIO.mMidnaIconPikariBackInner,
                   g_drawHIO.mMidnaIconPikariAnimSpeed,
                   3);
    }
}
...
void dMeter2Draw_c::initLife()
{
    ...
    static u64 const life_tag[] = {
        MULTI_CHAR('hpb_00'), MULTI_CHAR('hpb_01'), MULTI_CHAR('hpb_02'), MULTI_CHAR('hpb_03'), MULTI_CHAR('hpb_04'),
        MULTI_CHAR('hpb_05'), MULTI_CHAR('hpb_06'), MULTI_CHAR('hpb_07'), MULTI_CHAR('hpb_08'), MULTI_CHAR('hpb_09'),
        MULTI_CHAR('hpb_10'), MULTI_CHAR('hpb_11'), MULTI_CHAR('hpb_12'), MULTI_CHAR('hpb_13'), MULTI_CHAR('hpb_14'),
        MULTI_CHAR('hpb_15'), MULTI_CHAR('hpb_16'), MULTI_CHAR('hpb_17'), MULTI_CHAR('hpb_18'), MULTI_CHAR('hpb_19'),
    };

    static u64 const mark_tag[] = {
        MULTI_CHAR('heartn00'), MULTI_CHAR('heartn01'), MULTI_CHAR('heartn02'), MULTI_CHAR('heartn03'), MULTI_CHAR('heartn04'),
        MULTI_CHAR('heartn05'), MULTI_CHAR('heartn06'), MULTI_CHAR('heartn07'), MULTI_CHAR('heartn08'), MULTI_CHAR('heartn09'),
        MULTI_CHAR('heartn10'), MULTI_CHAR('heartn11'), MULTI_CHAR('heartn12'), MULTI_CHAR('heartn13'), MULTI_CHAR('heartn14'),
        MULTI_CHAR('heartn15'), MULTI_CHAR('heartn16'), MULTI_CHAR('heartn17'), MULTI_CHAR('heartn18'), MULTI_CHAR('heartn19'),
    };

    static u64 const base_tag[] = {
        MULTI_CHAR('h_ba_n00'), MULTI_CHAR('h_ba_n01'), MULTI_CHAR('h_ba_n02'), MULTI_CHAR('h_ba_n03'), MULTI_CHAR('h_ba_n04'),
        MULTI_CHAR('h_ba_n05'), MULTI_CHAR('h_ba_n06'), MULTI_CHAR('h_ba_n07'), MULTI_CHAR('h_ba_n08'), MULTI_CHAR('h_ba_n09'),
        MULTI_CHAR('h_ba_n10'), MULTI_CHAR('h_ba_n11'), MULTI_CHAR('h_ba_n12'), MULTI_CHAR('h_ba_n13'), MULTI_CHAR('h_ba_n14'),
        MULTI_CHAR('h_ba_n15'), MULTI_CHAR('h_ba_n16'), MULTI_CHAR('h_ba_n17'), MULTI_CHAR('h_ba_n18'), MULTI_CHAR('h_ba_n19'),
    };

    static u64 const lifet1_tag[] = {
        MULTI_CHAR('hear_00s'), MULTI_CHAR('hear_01s'), MULTI_CHAR('hear_02s'), MULTI_CHAR('hear_03s'), MULTI_CHAR('hear_04s'),
        MULTI_CHAR('hear_05s'), MULTI_CHAR('hear_06s'), MULTI_CHAR('hear_07s'), MULTI_CHAR('hear_08s'), MULTI_CHAR('hear_09s'),
        MULTI_CHAR('hear_10s'), MULTI_CHAR('hear_11s'), MULTI_CHAR('hear_12s'), MULTI_CHAR('hear_13s'), MULTI_CHAR('hear_14s'),
        MULTI_CHAR('hear_15s'), MULTI_CHAR('hear_16s'), MULTI_CHAR('hear_17s'), MULTI_CHAR('hear_18s'), MULTI_CHAR('hear_19s'),
    };

    static u64 const lifet2_tag[] = {
        MULTI_CHAR('hear_00'), MULTI_CHAR('hear_01'), MULTI_CHAR('hear_02'), MULTI_CHAR('hear_03'), MULTI_CHAR('hear_04'),
        MULTI_CHAR('hear_05'), MULTI_CHAR('hear_06'), MULTI_CHAR('hear_07'), MULTI_CHAR('hear_08'), MULTI_CHAR('hear_09'),
        MULTI_CHAR('hear_10'), MULTI_CHAR('hear_11'), MULTI_CHAR('hear_12'), MULTI_CHAR('hear_13'), MULTI_CHAR('hear_14'),
        MULTI_CHAR('hear_15'), MULTI_CHAR('hear_16'), MULTI_CHAR('hear_17'), MULTI_CHAR('hear_18'), MULTI_CHAR('hear_19'),
    };
    ...
    drawLife(dComIfGs_getMaxLife(), dComIfGs_getLife(), g_drawHIO.mLifeGaugePosX + var_f1, g_drawHIO.mLifeGaugePosY + var_f2);
...
void dMeter2Draw_c::initMagic()
{
    ...
    mpMagicFrameR = new CPaneMgr(mpKanteraScreen, MULTI_CHAR('m_w_r_n'), 2, NULL);
    ...
    drawKantera(dComIfGs_getMaxOil(),
                dComIfGs_getOil(),
...
void dMeter2Draw_c::initLightDrop()
{
    ...
    drawLightDrop(dComIfGs_getLightDropNum(dComIfGp_getStartStageDarkArea()),
                  ...
                  g_drawHIO.mLightDrop.mVesselPosX,
                  g_drawHIO.mLightDrop.mVesselPosY,
                  g_drawHIO.mLightDrop.mVesselScale,
                  g_drawHIO.mLightDrop.mVesselAlpha[0],
                  0);
...
void dMeter2Draw_c::initRupeeKey()
{
    ...
    static u64 const rupeet1_tag[] = {MULTI_CHAR('r_n_1_s'),
                                      MULTI_CHAR('r_n_2_s'),
                                      MULTI_CHAR('r_n_3_s'),
                                      MULTI_CHAR('r_n_4_s')};
    ...
    static u64 const key_tag[] = {MULTI_CHAR('key_nul'),
                                  MULTI_CHAR('k_n_1_n'),
                                  MULTI_CHAR('k_n_2_n'),
                                  MULTI_CHAR('k_n_3_n'),
                                  MULTI_CHAR('k_n_4_n')};
...
void dMeter2Draw_c::initButton()
{
    ...
    mpItemBPane = new J2DPicture(MULTI_CHAR('b_itm_pp'),
                                 JGeometry::TBox2<f32>(0.0f, 0.0f, mpItemB->getInitSizeX(), mpItemB->getInitSizeY()),
                                 static_cast<J2DPicture*>(mpItemB->getPanePtr())->getTexture(0)->getTexInfo(),
                                 NULL);
    ...
    mpItemXYPane[0] =
        new J2DPicture(MULTI_CHAR('x_itm_pp'),
                       JGeometry::TBox2<f32>(0.0f, 0.0f, mpItemXY[0]->getInitSizeX(), mpItemXY[0]->getInitSizeY()),
                       static_cast<J2DPicture*>(mpItemXY[0]->getPanePtr())->getTexture(0)->getTexInfo(),
                       NULL);
    ...
    mpItemXYPane[1] =
        new J2DPicture(MULTI_CHAR('y_itm_pp'),
                       JGeometry::TBox2<f32>(0.0f, 0.0f, mpItemXY[1]->getInitSizeX(), mpItemXY[1]->getInitSizeY()),
                       static_cast<J2DPicture*>(mpItemXY[1]->getPanePtr())->getTexture(0)->getTexInfo(),
                       NULL);
    ...
    mpItemXYPane[2] = new J2DPicture(MULTI_CHAR('r_itm_pp'),
                                     JGeometry::TBox2<f32>(0.0f, 0.0f, mpItemR->getInitSizeX(), mpItemR->getInitSizeY()),
                                     static_cast<J2DPicture*>(mpItemR->getPanePtr())->getTexture(0)->getTexInfo(),
                                     NULL);
    ...
    static u64 const juji_i_tag[] = {MULTI_CHAR('ju_ring4'),
                                     MULTI_CHAR('yaji_00'),
                                     MULTI_CHAR('yaji_01'),
                                     MULTI_CHAR('ju_ring2'),
                                     MULTI_CHAR('ju_ring4')};
    static u64 const juji_m_tag[] = {MULTI_CHAR('ju_ring3'),
                                     MULTI_CHAR('yaji_02'),
                                     MULTI_CHAR('yaji_03'),
                                     MULTI_CHAR('ju_ring3'),
                                     0};
...
void dMeter2Draw_c::initButtonCross()
{
    ...
    dMeter2Info_getString(0x61, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('cont_ju0')))->getStringPtr(), NULL);
    dMeter2Info_getString(0x61, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('cont_ju1')))->getStringPtr(), NULL);
    dMeter2Info_getString(0x61, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('cont_ju2')))->getStringPtr(), NULL);
    dMeter2Info_getString(0x61, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('cont_ju3')))->getStringPtr(), NULL);
    dMeter2Info_getString(0x61, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('cont_ju4')))->getStringPtr(), NULL);
    ...
    dMeter2Info_getString(0x62, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('cont_ju5')))->getStringPtr(), NULL);
    dMeter2Info_getString(0x62, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('cont_ju6')))->getStringPtr(), NULL);
    dMeter2Info_getString(0x62, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('cont_ju7')))->getStringPtr(), NULL);
    dMeter2Info_getString(0x62, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('cont_ju8')))->getStringPtr(), NULL);
    dMeter2Info_getString(0x62, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('cont_ju9')))->getStringPtr(), NULL);
                               ...
                               u8 param_8)
{
    ...
    drawPikari(pos.x, pos.y, i_framep, i_scale, i_moyaBlack, i_moyaWhite, i_moyabsBlack, i_moyabsWhite, param_7, param_8);
}
                               ...
                               u8 param_9)
{
    ...
    else
    {
        *i_framep += param_8;
        ...
        mpPikariParent->scale(i_scale * mpPikariParent->getScaleX(), i_scale * mpPikariParent->getScaleY());

        if (mpPikariScreen->search(MULTI_CHAR('moya00'))->isVisible())
        {
            static_cast<J2DPicture*>(mpPikariScreen->search(MULTI_CHAR('moya00')))->setBlackWhite(i_moyaBlack, i_moyaWhite);
        }

        if (mpPikariScreen->search(MULTI_CHAR('moyabs'))->isVisible())
        {
            static_cast<J2DPicture*>(mpPikariScreen->search(MULTI_CHAR('moyabs')))->setBlackWhite(i_moyabsBlack, i_moyabsWhite);
        }

        if (param_9 != 4)
        {
            static_cast<J2DPicture*>(mpPikariScreen->search(MULTI_CHAR('pika00')))->setBlackWhite(mPikariBlack, mPikariBlack);
        }
        else
        {
...
void dMeter2Draw_c::changeTextureLife(int i_no, bool param_1, u8 i_quarterNum)
{
    if (param_1)
    {
        ...
        else
        {
            ...
            static u64 const tag_bigh[] = {MULTI_CHAR('bigh_00'),
                                           MULTI_CHAR('bigh_01'),
                                           MULTI_CHAR('bigh_02'),
                                           MULTI_CHAR('bigh_03')};
            ...
            mpBigHeart->translate(mpLifeParts[i_no]->getTranslateX() + (mpScreen->search(tag_du[i_no / 10])->getTranslateX() -
                                                                        mpScreen->search(tag_du[0])->getTranslateX()),
                                  mpScreen->search(tag_du[i_no / 10])->getTranslateY() + mpLifeParts[i_no]->getTranslateY());
        }
    }
    else
    {
...
{
    static u64 const lifet2_tag[] = {
        'hear_00', 'hear_01', 'hear_02', 'hear_03', 'hear_04', 'hear_05', 'hear_06', 'hear_07', 'hear_08', 'hear_09',
        'hear_10', 'hear_11', 'hear_12', 'hear_13', 'hear_14', 'hear_15', 'hear_16', 'hear_17', 'hear_18', 'hear_19',
    };
...
void dMeter2Draw_c::drawKanteraScreen(u8 i_meterType)
{
    ...
    else if (i_meterType == 1)
    {
        ...
        {
            mpMagicMeter->setBlackWhite(JUtility::TColor(255, 255, 140, 255), JUtility::TColor(230, 170, 0, 255));
        }
        setAlphaKanteraChange(true);
    }
    else if (i_meterType == 2)
    {
        ...
        {
            mpMagicMeter->setBlackWhite(JUtility::TColor(255, 100, 100, 255), JUtility::TColor(255, 10, 10, 255));
            playOxygenBpkAnimation(mpOxygenBpk[0]);

            if (mMeterAlphaRate[i_meterType] > 0.0f)
            {
                Z2GetAudioMgr()->seStartLevel(Z2SE_SWIM_TIMER_BLINK_2, NULL, 0, 0, 1.0f, 1.0f, -1.0f, -1.0f, 0);
            }
        }
        else if (oxygen_percent <= 0.5f)
        {
            mpMagicMeter->setBlackWhite(JUtility::TColor(200, 200, 255, 255), JUtility::TColor(80, 180, 255, 255));
            playOxygenBpkAnimation(mpOxygenBpk[1]);

            if (mMeterAlphaRate[i_meterType] > 0.0f)
            {
                Z2GetAudioMgr()->seStartLevel(Z2SE_SWIM_TIMER_BLINK_1, NULL, 0, 0, 1.0f, 1.0f, -1.0f, -1.0f, 0);
            }
        }
        else
        {
...
void dMeter2Draw_c::drawLightDrop(u8 i_num, u8 i_needNum, f32 i_posX, f32 i_posY, f32 i_vesselScale, f32 param_5, u8 param_6)
{
    for (int i = 0; i < 16; i++)
    {
        if (mpSIParts[i][0] != NULL)
        {
            mpSIParts[i][0]->scale(g_drawHIO.mLightDrop.mDropScale, g_drawHIO.mLightDrop.mDropScale);
        }
        else
        {
            mpSIParts[i][1]->scale(g_drawHIO.mLightDrop.mDropScale, g_drawHIO.mLightDrop.mDropScale);
            mpSIParts[i][2]->scale(g_drawHIO.mLightDrop.mDropScale, g_drawHIO.mLightDrop.mDropScale);
        }

        if (i < i_num)
        {
            static_cast<J2DPicture*>(mpSIParts[i][1]->getPanePtr())
                ->setBlackWhite(g_drawHIO.mLightDrop.mDropOnColorBlack, g_drawHIO.mLightDrop.mDropOnColorWhite);
        ...
        {
            static_cast<J2DPicture*>(mpSIParts[i][1]->getPanePtr())
                ->setBlackWhite(g_drawHIO.mLightDrop.mDropOffColorBlack, g_drawHIO.mLightDrop.mDropOffColorWhite);
    ...
    mpLightDropParent->scale(mLightDropVesselScale * field_0x6f8, mLightDropVesselScale * field_0x6f8);
...
void dMeter2Draw_c::setAlphaLightDropAnimeMin()
{
    if (mpLightDropParent->getAlphaRate() != 0.0f)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            mpSIParent[i]->setAlphaRate(g_drawHIO.mLightDrop.mVesselAlpha[i + 2] * mpLightDropParent->getAlphaRate());
        }
...
void dMeter2Draw_c::setAlphaLightDropAnimeMax()
{
        ...
        dMeter2Info_getLightDropGetFlag(dComIfGp_getStartStageDarkArea()) != 0xFF)
    {
        ...
        {
            field_0x6f8 =
                1.0f + (((g_drawHIO.mLightDrop.mDropGetScale - 1.0f) *
                         ((f32)g_drawHIO.mLightDrop.mDropGetScaleAnimFrameNum - (f32)mpLightDropParent->getAlphaTimer())) /
...
void dMeter2Draw_c::setAlphaRupeeChange(bool param_0)
{
    ...
    if (set_parent || set_rupeekey || set_rupeecount || param_0)
    {
        for (int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                mpRupeeTexture[i][j]->setAlphaRate(field_0x7d0 * (field_0x7cc * (mRupeeCountAlpha * mRupeeAlpha)));
            }
        }
    }
}
                                ...
                                bool param_7)
{
    ...
    if (*mp_string != 0 &&
        ((dComIfGp_getDoStatus() != dComIfGp_getAStatus() && dComIfGp_getDoStatus() != dComIfGp_getAStatusForce()) ||
         !isEmphasisA()))
    {
        mpTextA->show();
        mpTextA->show();
                                ...
                                bool param_7)
{
    ...
    mpItemB->getPanePtr()->rotate(mpItemB->getSizeX() * 0.5f,
                                  mpItemB->getSizeY() * 0.5f,
                                  ROTATE_Z,
...
void dMeter2Draw_c::drawButtonZ(u8 i_action)
{
    ...
    mpItemR->paneTrans(g_drawHIO.mButtonZItemPosX + field_0x6ac[2], g_drawHIO.mButtonZItemPosY + field_0x6b8[2]);
...
void dMeter2Draw_c::drawButtonXY(int i_no, u8 i_itemNo, u8 i_action, bool param_3, bool param_4)
{
    ...
    if (!param_3)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            strcpy(static_cast<J2DTextBox*>(mpXYText[i][i_no]->getPanePtr())->getStringPtr(), mp_string);
        }
    ...
    else
    {
        ...
        if (i_no == SELECT_X_e)
        {
            ...
            mpLightXY[0]->scale(g_drawHIO.mButtonXItemBaseScale[var_r29], g_drawHIO.mButtonXItemBaseScale[var_r29]);
            mpLightXY[0]->paneTrans(g_drawHIO.mButtonXItemBasePosX[var_r29], g_drawHIO.mButtonXItemBasePosY[var_r29]);
        ...
        else if (i_no == SELECT_Y_e)
        {
            ...
            mpLightXY[1]->scale(g_drawHIO.mButtonYItemBaseScale[var_r29], g_drawHIO.mButtonYItemBaseScale[var_r29]);
            mpLightXY[1]->paneTrans(g_drawHIO.mButtonYItemBasePosX[var_r29], g_drawHIO.mButtonYItemBasePosY[var_r29]);
...
void dMeter2Draw_c::setAlphaButtonChange(bool param_0)
{
    ...
    if (mButtonATextColor.r != g_drawHIO.mButtonATextColor.r || mButtonATextColor.g != g_drawHIO.mButtonATextColor.g ||
        mButtonATextColor.b != g_drawHIO.mButtonATextColor.b || mButtonATextColor.a != g_drawHIO.mButtonATextColor.a || param_0)
    {
        mButtonATextColor.r = g_drawHIO.mButtonATextColor.r;
        mButtonATextColor.g = g_drawHIO.mButtonATextColor.g;
        mButtonATextColor.b = g_drawHIO.mButtonATextColor.b;
    ...
    if (mButtonBTextColor.r != g_drawHIO.mButtonBFontColor.r || mButtonBTextColor.g != g_drawHIO.mButtonBFontColor.g ||
        mButtonBTextColor.b != g_drawHIO.mButtonBFontColor.b || mButtonBTextColor.a != g_drawHIO.mButtonBFontColor.a || param_0)
    {
        mButtonBTextColor.r = g_drawHIO.mButtonBFontColor.r;
        mButtonBTextColor.g = g_drawHIO.mButtonBFontColor.g;
        mButtonBTextColor.b = g_drawHIO.mButtonBFontColor.b;
    ...
    if (mButtonZTextColor.r != g_drawHIO.mButtonZFontColor.r || mButtonZTextColor.g != g_drawHIO.mButtonZFontColor.g ||
        mButtonZTextColor.b != g_drawHIO.mButtonZFontColor.b || mButtonZTextColor.a != g_drawHIO.mButtonZFontColor.a || param_0)
    {
        mButtonZTextColor.r = g_drawHIO.mButtonZFontColor.r;
        mButtonZTextColor.g = g_drawHIO.mButtonZFontColor.g;
        mButtonZTextColor.b = g_drawHIO.mButtonZFontColor.b;
    ...
    if (mButtonXYTextColor.r != g_drawHIO.mButtonXYTextColor.r || mButtonXYTextColor.g != g_drawHIO.mButtonXYTextColor.g ||
        mButtonXYTextColor.b != g_drawHIO.mButtonXYTextColor.b || mButtonXYTextColor.a != g_drawHIO.mButtonXYTextColor.a ||
        param_0)
    {
        mButtonXYTextColor.r = g_drawHIO.mButtonXYTextColor.r;
        mButtonXYTextColor.g = g_drawHIO.mButtonXYTextColor.g;
...
void dMeter2Draw_c::setAlphaButtonAAnimeMin()
{
    ...
    {
        mpButtonA->setAlphaRate(g_drawHIO.mButtonAAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha));
...
{
    if (g_drawHIO.mButtonAAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha) != mpButtonA->getAlphaRate())
    {
        mpButtonA->setAlphaRate(g_drawHIO.mButtonAAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha));
...
void dMeter2Draw_c::setAlphaButtonBAnimeMin()
{
    ...
    {
        mpButtonB->setAlphaRate(g_drawHIO.mButtonBAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha));
        setAlphaAnimeMin(mpButtonB, 5);

        mpLightB->setAlphaRate(g_drawHIO.mButtonBAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha));
...
{
    if (mpButtonB->getAlphaRate() != g_drawHIO.mButtonBAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha))
    {
        mpButtonB->setAlphaRate(g_drawHIO.mButtonBAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha));
...
void dMeter2Draw_c::setButtonIconAAlpha(u8 unused0, u32 unused1, bool unused2)
{
    ...
    {
        u8 alpha = g_drawHIO.mButtonAAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha) *
...
void dMeter2Draw_c::setButtonIconBAlpha(u8 unused0, u32 unused1, bool param_2)
{
    ...
    {
        f32 temp_f30 = g_drawHIO.mButtonBAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha);
...
void dMeter2Draw_c::setButtonIconMidonaAlpha(u32 param_0)
{
    ...
    {
        f32 temp_f30 = g_drawHIO.mMidnaIconAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha);
    ...
    f32 var_f29_2 = (g_drawHIO.mButtonZAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha) *
...
void dMeter2Draw_c::setButtonIconAlpha(int i_no, u8 unused0, u32 unused1, bool unused2)
{
    ...
    if (mpItemXY[i_no]->isVisible() || mpLightXY[i_no]->isVisible() || mpButtonXY[i_no]->isVisible())
    {
        f32 var_f30 = 1.0f;
        ...
        if (i_no == 0)
        {
            var_f2 = g_drawHIO.mButtonXAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha);
            var_f30 = g_drawHIO.mButtonXItemBaseAlpha[var_r26];
        }
        else if (i_no == 1)
        {
            var_f2 = g_drawHIO.mButtonYAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha);
            var_f30 = g_drawHIO.mButtonYItemBaseAlpha[var_r26];
        }
        else
        {
            var_f2 = g_drawHIO.mButtonZAlpha * (g_drawHIO.mParentAlpha * g_drawHIO.mMainHUDButtonsAlpha);
        }
...
{
    return (ResTIMG*)dComIfGp_getMain2DArchive()->getResource('TIMG', dMeter2Info_getNumberTextureName(i));
}

char* dMeter2Draw_c::getActionString(u8 i_action, u8 i_type, u8* param_2)
{
    static u32 i_action_num[] = {
        0,    1001, 1002, 1003, 1004, 1005, 1006, 1007, 1387, 1009, 1010, 1011, 1012, 1013, 1014, 1015, 1033, 1016, 1017,
        1018, 1019, 1020, 1045, 1022, 1026, 1027, 1028, 1029, 1030, 1024, 1031, 1025, 1023, 1045, 1036, 1032, 93,   0,
        0,    0,    1035, 1034, 1037, 1038, 1039, 1040, 1041, 998,  1042, 1043, 1044, 1046, 1047, 1048, 1049, 1050, 1051,
        1052, 1053, 1054, 1055, 1056, 1057, 1058, 1059, 1070, 1060, 1061, 1062, 1063, 1064, 1067, 1065, 1066, 1211, 1069,
        1021, 1072, 1073, 0,    1074, 1075, 1076, 1077, 1078, 1079, 1080, 1081, 1082, 1083, 1084, 1085, 1086, 1087, 1088,
        1089, 1090, 1092, 1093, 1094, 1095, 1096, 1904, 1097, 1098, 1099, 1100, 1150, 1149, 1148, 1377, 1147, 1145, 1146,
        1161, 1162, 1163, 1164, 1165, 1166, 1144, 982,  983,  1143, 1160, 1319, 1314, 1399, 1008,
    };
...
void dMeter2Draw_c::changeTextureItemB(u8 i_itemNo)
{
    ...
    if (dMeter2Info_readItemTexture(i_itemNo,
                                    mpItemBTex[field_0x76b][0],
                                    (J2DPicture*)mpItemB->getPanePtr(),
                                    mpItemBTex[field_0x76b][1],
                                    mpItemBPane,
                                    NULL,
                                    NULL,
                                    NULL,
...
void dMeter2Draw_c::changeTextureItemXY(int i_no, u8 i_itemNo)
{
    ...
    if (dMeter2Info_readItemTexture(i_itemNo,
                                    mpItemXYTex[i_no][field_0x76c[i_no]][0],
                                    (J2DPicture*)mpItemXY[i_no]->getPanePtr(),
                                    mpItemXYTex[i_no][field_0x76c[i_no]][1],
                                    mpItemXYPane[i_no],
                                    NULL,
                                    NULL,
                                    NULL,
    ...
    field_0x6c4[i_no] = var_f4 * ((mpItemXYTex[i_no][field_0x76c[i_no]][0]->width * mpItemXY[i_no]->getInitSizeX()) / 48.0f);
    field_0x6d0[i_no] = var_f4 * ((mpItemXYTex[i_no][field_0x76c[i_no]][0]->height * mpItemXY[i_no]->getInitSizeY()) / 48.0f);
...
void dMeter2Draw_c::setItemNum(u8 i_button, u8 i_num, u8 i_max)
{
    ...
    {
        ResTIMG* timg =
            (ResTIMG*)dComIfGp_getMain2DArchive()->getResource('TIMG', dMeter2Info_getNumberTextureName(i_num / 10));
        mpItemNumTex[i_button][0]->changeTexture(timg, 0);

        timg = (ResTIMG*)dComIfGp_getMain2DArchive()->getResource('TIMG', dMeter2Info_getNumberTextureName(i_num % 10));
    ...
    {
        ResTIMG* timg =
            (ResTIMG*)dComIfGp_getMain2DArchive()->getResource('TIMG', dMeter2Info_getNumberTextureName(i_num / 100));
        mpItemNumTex[i_button][0]->changeTexture(timg, 0);
...
void dMeter2Draw_c::drawKanteraMeter(u8 i_button, f32 i_alphaRate)
{
    ...
    mpKanteraMeter[i_button]->setPos(((vtx0.x + vtx3.x) * 0.5f) + 9.0f + sp10[i_button], vtx3.y + sp8[i_button]);
...
// unused
static const char* tex_name[] = {"tt_heart_00.bti", "tt_heart_01.bti", "tt_heart_02.bti", "tt_heart_03.bti"};

u8 dMeter2Draw_c::getCanoeFishing()
{
    if (!strcmp(dComIfGp_getStartStageName(), "F_SP127") && ((daPy_py_c*)dComIfGp_getPlayer(0))->checkCanoeRide())
    {
        return TRUE;
    }
    else
