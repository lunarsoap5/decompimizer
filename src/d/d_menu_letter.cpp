...
void dMenu_Letter_c::_draw()
{
    ...
    mpBlackTex->draw(mDoGph_gInf_c::getMinXF(),
    ...
    grafContext->scissor(left, top, width, height);
    grafContext->setScissor();
    if (mProcess != 2 && mProcess != 1)
    {
        mpDrawCursor->draw();
    ...
    if (mProcess == 3 || mProcess == 4 || mProcess == 5 || mProcess == 6 || mProcess == 7)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            mpString->getStringPage(dMenu_Letter::getLetterText(dVar1),
                                    field_0x3e3 - 1,
                                    D_MENU_LETTER_LINE_MAX,
                                    uVar18,
                                    uVar10,
                                    NULL,
                                    NULL,
...
void dMenu_Letter_c::read_open_init()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        mpString->getStringPage(dMenu_Letter::getLetterText(idx),
                                field_0x3e3 - 1,
                                D_MENU_LETTER_LINE_MAX,
                                text2,
                                text1,
...
void dMenu_Letter_c::setHIO(bool i_useHIO)
{
    if (i_useHIO || g_drawHIO.mLetterSelectScreen.mDebugON)
    {
        for (int i = 0; i < 3; i++)
        {
            mpParent[i]->paneTrans(g_drawHIO.mLetterSelectScreen.mWindowPosX, g_drawHIO.mLetterSelectScreen.mWindowPosY);
            mpParent[i]->scale(g_drawHIO.mLetterSelectScreen.mWindowScale, g_drawHIO.mLetterSelectScreen.mWindowScale);
        }
        mpDMYParent->paneTrans(g_drawHIO.mLetterSelectScreen.mWindowPosX, g_drawHIO.mLetterSelectScreen.mWindowPosY);
        mpDMYParent->scale(g_drawHIO.mLetterSelectScreen.mWindowScale, g_drawHIO.mLetterSelectScreen.mWindowScale);
        for (int i = 0; i < 9; i++)
        {
        ...
        mpLineParent->paneTrans(g_drawHIO.mLetterSelectScreen.mLetterLinePosX, g_drawHIO.mLetterSelectScreen.mLetterLinePosY);
        for (int i = 0; i < 12; i++)
        {
        ...
        ((J2DTextBox*)field_0x2ec[1]->getPanePtr())
            ->setBlackWhite(g_drawHIO.mLetterSelectScreen.mLetterTextBack, g_drawHIO.mLetterSelectScreen.mLetterTextFront);
    }
