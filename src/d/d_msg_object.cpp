...
int dMsgObject_c::_draw()
{
    ...
    if (sVar7 != 0 && sVar7 != 1 && sVar7 != 0xb)
    {
        ...
        if (mpScrnDraw != NULL)
        {
            ...
            mpScrnDraw->setSelectString(pRef->getSelTextPtr(0), pRef->getSelTextPtr(1), pRef->getSelTextPtr(2));
            mpScrnDraw->setSelectRubyString(pRef->getSelRubyPtr(0), pRef->getSelRubyPtr(1), pRef->getSelRubyPtr(2));
        }
    }
    if (mpScrnDraw != NULL)
    {
...
void dMsgObject_c::waitProc()
{
    if (mNoDemoFlag || field_0x4cc)
    {
        ...
        {
            if (dComIfGp_checkCameraAttentionStatus(dComIfGp_getPlayerCameraID(0), 4) || dMsgObject_isCameraCancelFlag() ||
...
void dMsgObject_c::openProc()
{
    if (isMidonaMessage())
    {
        ...
        if (field_0x16a == 0)
        {
            ...
            {
                mpScrnDraw->selectAnimeInit(3, pRef->getSelectPos(), pRef->getSelTBoxWidth(), pRef->getSelectRubyFlag());
            }
            else
            {
                mpScrnDraw->selectAnimeInit(2, pRef->getSelectPos() + 1, pRef->getSelTBoxWidth(), pRef->getSelectRubyFlag());
            }
            pRef->setStopFlag(0);
        }
        else
    ...
    {
        sVar7 = g_MsgObject_HIO_c.mBoxAppearFrame + g_MsgObject_HIO_c.mWaitFrame + g_MsgObject_HIO_c.mLightAppearFrame;
...
{
    jmessage_tReference* pRef = (jmessage_tReference*)mpRenProc->getReference();
    if (pRef->getSendFlag() != 1 && pRef->getSendFlag() != 3 && pRef->getSendFlag() != 4 && pRef->getSendFlag() != 6)
    {
        mpScrnDraw->arwAnimeMove();
    }
    if (((mDoCPd_c::getTrigA(0) == 0 && mDoCPd_c::getTrigB(0) == 0) || !isHowlMessage()) && isSend())
    {
        mpCtrl->render_synchronize();
        mpCtrl->reset();
        mpCtrl->resetResourceCache();
...
void dMsgObject_c::selectProc()
{
    ...
    else if (mDoCPd_c::getTrigB(0) && (((mpScrnDraw->isSelect() || getStatusLocal() == 20) && getSelectCancelPos() != 0)))
    {
        setSelectCursorPosLocal(getSelectCancelPos() - 1);
        uVar7 = true;
        if (getStatusLocal() == 8)
        ...
        if (mDoCPd_c::getTrigA(0))
        {
            ...
            if (iVar2)
            {
                if (getInputValue() > 0)
                {
                    ...
                    {
                        /* dSv_event_flag_c::F_0376 - Kakariko Village - Gathered funds for bridge repair! (set by program after
                         * raising funds) */
            ...
            else
            {
                ...
                // Handle case for if trill is goal
                if ((g_seedInfo.getHeaderPtr()->getCustomGoal() == Goal_Trill) &&
                    dMsgObject_getTotalPayment() >= g_seedInfo.getHeaderPtr()->getTrillDonationCount())
                {
                    rollCredits();
                }
...
void dMsgObject_c::finishProc()
{
    ...
    u8 sendRes = isSend();
    if (((mDoCPd_c::getTrigA(0) == 0 && (mDoCPd_c::getTrigB(0) == 0 || ((dMsgScrnHowl_c*)mpScrnDraw)->isKeyCheck())) ||
         !isHowlMessage()) &&
        sendRes != 0)
    {
...
void dMsgObject_c::talkStartInit()
{
    ...
    if (((dComIfGp_isHeapLockFlag() == 0 || (dComIfGp_isHeapLockFlag() == 5 && mFukiKind != 15)) ||
         (((dComIfGp_isHeapLockFlag() == 2 || (dComIfGp_isHeapLockFlag() == 3 || dComIfGp_isHeapLockFlag() == 1)) &&
    ...
    else
    {
        ...
        else if (bVar4 == 3)
        {
            ...
            else
            {
                ...
                if (local_3c.x >= 0.0f && local_3c.x <= FB_WIDTH && local_3c.y >= 0.0f && local_3c.y <= FB_HEIGHT)
                {
...
u8 dMsgObject_c::isSend()
{
    ...
    if (pRef->getSendFlag() != 0)
    {
        ...
        if (pRef->getSendFlag() == 5)
        {
            ...
            {
                setButtonStatusLocal();
                if ((g_seedInfo.isInstantText() && mDoCPd_c::getHoldB(0)) ||
...
bool dMsgObject_c::getStringLocal(u32 param_1,
                                  J2DTextBox* param_2,
                                  J2DTextBox* param_3,
                                  JUTFont* param_4,
                                  COutFont_c* param_5,
...
{
    if (mFukiKind == 13 && (mpRefer->getMsgID() == 0x7d3 || mpRefer->getMsgID() == 0x7f6 || mpRefer->getMsgID() == 0x7fa))
    {
        return 1;
    }
    return 0;
...
{
    if (isKanbanMessage() || isPlaceMessage() || isBossMessage() || isBookMessage() || isHowlMessage() || isStaffMessage())
    {
        return false;
    }
    return true;
...
{
    jmessage_tReference* pRef = (jmessage_tReference*)mpRenProc->getReference();
    if (field_0x168 == getStatusLocal() && (pRef->getSendFlag() != 1 && pRef->getSendFlag() != 3) && pRef->getSendFlag() != 4 &&
        pRef->getSendFlag() != 6 && pRef->getSendFlag() != 2)
    {
        ...
        else if (getStatusLocal() == 8 || getStatusLocal() == 9 || getStatusLocal() == 20)
        {
...
void dMsgObject_c::setSelectCursorPosLocal(u8 param_1)
{
    ...
    {
        Z2GetAudioMgr()->seStart(daAlink_c::WANM_ATTACK_POE, NULL, 1, 0, 1.0f, 1.0f, -1.0f, -1.0f, 0);
    }
...
bool dMsgObject_c::getString(u32 param_0,
                             J2DTextBox* param_1,
                             J2DTextBox* param_2,
                             JUTFont* param_3,
                             COutFont_c* param_4,
                             ...
                             s16* param_8)
{
    return dMsgObject_getMsgObjectClass()
        ->getStringLocal(param_0, param_1, param_2, param_3, param_4, param_5, param_6, param_7, param_8);
}
