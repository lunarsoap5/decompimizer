...
{
    if (c != 0x8140 && c != 0x8141 && c != 0x8142 && c != 0x0020 && c != 0x0022 && c != 0x0027 && c != 0x002C && c != 0x002E)
    {
        return true;
    }
...
static u16 change1ByteTo2Bytes(int iCharacter)
{
    static u16 char_table[] = {
        ...
        0x8384, 0x8386, 0x8388, 0x8389, 0x838A, 0x838B, 0x838C, 0x838D, 0x838F, 0x8393, 0x814A, 0x814B,
    };
...
jmessage_tMeasureProcessor::jmessage_tMeasureProcessor(jmessage_tReference const* pReference): TRenderingProcessor(pReference)
{
...
void jmessage_tMeasureProcessor::do_end()
{
    ...
    Z2GetSpeechMgr2()->setString(pReference->getCharSoundInfo().data,
                                 pReference->getCharSoundInfo().field_0x40c,
                                 mSeSpeaker,
                                 mSeMood);
}
...
void jmessage_tMeasureProcessor::do_character(int iCharacter)
{
    ...
    else
    {
        ...
        if (field_0x46 != 0)
        {
            ...
            pReference->addSelLength(field_0x46 - 1,
                                     (var_f31 * (field_0x38 * font->getWidth(iCharacter))) + pReference->getSelCharSpace());
        }
...
jmessage_tSequenceProcessor::jmessage_tSequenceProcessor(jmessage_tReference const* pReference, jmessage_tControl* pControl):
    JMessage::TSequenceProcessor(pReference, pControl), mMeasureProcessor(pReference)
{
    field_0xa8 = 1;
...
bool jmessage_tSequenceProcessor::do_isReady()
{
    ...
    if (dComIfGp_checkMesgBgm())
    {
        ...
        if (mDoAud_checkPlayingSubBgmFlag() != Z2BGM_ITEM_GET && mDoAud_checkPlayingSubBgmFlag() != Z2BGM_ITEM_GET_MINI)
        {
            dComIfGp_setMesgBgmOff();
    ...
    if (pReference->isKanban() || pReference->isPlaceName() || pReference->isStaffRoll() || pReference->isBossName() ||
        pReference->isSaveSeq() || pReference->isHowl())
    {
    ...
    {
        if (pReference->getStatus() == 2 || pReference->getStatus() == 6)
        {
        ...
        if (field_0x14c != 0)
        {
            ...
            if (field_0x14c == 0)
            {
                ...
                else
                {
                    ...
                    do_rubystrcat(field_0x152, pReference->getRubyPtr(), pReference->getRubyCharSpace(), var_f22);
                }
            }
        }
...
void jmessage_tRenderingProcessor::do_heightcenter()
{
    ...
    if (field_0x142 == 0)
    {
        var_f31 = (pReference->getTBoxHeight() -
                   (pReference->getFontSizeY() + (pReference->getLineSpace() * (pReference->getLineMax() - 1)))) /
                  2;
    }
    else
    {
...
void jmessage_tRenderingProcessor::do_outfont(u8 i_iconNo, u32 i_color)
{
    ...
    if (mCharInfoPtr != NULL)
    {
        getCharInfo(0.5f + (posX + (0.5f * sizeX)), field_0x138 + (posY + (0.5f * sizeY)), field_0x44, field_0x44, 1.0f);
    }
...
void jmessage_tRenderingProcessor::do_arrow2()
{
    ...
    if (field_0x14d == pReference->getSelectPos())
    {
        mpOutFont->drawFont(NULL,
                            ...
                            field_0x4c,
                            (dVar11 * (field_0x44 * pReference->getFontSizeX())),
                            dVar11 * (field_0x44 * pReference->getFontSizeY()),
                            mCCColor,
                            0xFF);
    }
    ...
    if (pReference->isCharSend() || !param_2)
    {
        ...
        else if (field_0x11c < D_MSG_CLASS_CHAR_CNT_MAX)
        {
            if (param_2)
            {
                ...
                else if (field_0x146 > pReference->getCharCnt() ||
                         (field_0x146 == pReference->getCharCnt() && pReference->getCharAlpha() < 255.0f))
                {
...
jmessage_string_tMeasureProcessor::jmessage_string_tMeasureProcessor(jmessage_string_tReference const* pReference):
    JMessage::TRenderingProcessor(pReference)
{
...
jmessage_string_tSequenceProcessor::jmessage_string_tSequenceProcessor(jmessage_string_tReference const* pReference,
...
jmessage_string_tRenderingProcessor::jmessage_string_tRenderingProcessor(jmessage_string_tReference const* pReference):
    JMessage::TRenderingProcessor(pReference)
{
    mpReference = (jmessage_string_tReference*)getReference();
    do_reset();
    ...
    if (getLineCountNowPage() >= 0 && mpReference->getPanePtr() != NULL)
    {
        ...
        if (mpReference->getPanePtr()->getHBinding() == HBIND_CENTER)
        {
            snprintf(buffer,
                     ...
                     "\x1B"
                     "CL[%d]",
                     (int)mpReference->getOutfontLength(getLineCountNowPage()));
            do_strcat(buffer);
        }
...
void jmessage_string_tRenderingProcessor::do_outfont(u8 i_iconNo)
{
    if (mpReference->getPanePtr() != NULL)
    {
        ...
        if (mpReference->getOutFontPtr() != NULL)
        {
            ...
            mpReference->getOutFontPtr()->drawFont(mpReference->getPanePtr(),
                                                   i_iconNo,
                                                   field_0x4c + posX,
                                                   field_0x50 + posY,
                                                   fontSize.mSizeX,
