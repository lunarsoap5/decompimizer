...
static initFunc init[] = {&dMenu_Collect2D_c::wait_init,
...
static processFunc process[] = {
    &dMenu_Collect2D_c::wait_proc,          &dMenu_Collect2D_c::save_open_proc,    &dMenu_Collect2D_c::save_move_proc,
    &dMenu_Collect2D_c::save_close_proc,    &dMenu_Collect2D_c::option_open_proc,  &dMenu_Collect2D_c::option_move_proc,
    &dMenu_Collect2D_c::option_close_proc,  &dMenu_Collect2D_c::letter_open_proc,  &dMenu_Collect2D_c::letter_move_proc,
    &dMenu_Collect2D_c::letter_close_proc,  &dMenu_Collect2D_c::fishing_open_proc, &dMenu_Collect2D_c::fishing_move_proc,
    &dMenu_Collect2D_c::fishing_close_proc, &dMenu_Collect2D_c::skill_open_proc,   &dMenu_Collect2D_c::skill_move_proc,
    &dMenu_Collect2D_c::skill_close_proc,   &dMenu_Collect2D_c::insect_open_proc,  &dMenu_Collect2D_c::insect_move_proc,
...
{
    if (dComIfGs_getFishNum(0) || dComIfGs_getFishNum(1) || dComIfGs_getFishNum(2) || dComIfGs_getFishNum(3) ||
        dComIfGs_getFishNum(4) || dComIfGs_getFishNum(5) || g_drawHIO.mFishListScreen.mDebug)
    {
        return true;
    }
    return false;
...
void dMenu_Collect2D_c::screenSet()
{
    ...
    dMeter2Info_getStringKanji(0x3E1, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('t_t00')))->getStringPtr(), NULL);
    ...
    dMeter2Info_getStringKanji(0x3E1, static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('f_t00')))->getStringPtr(), NULL);
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        dMeter2Info_getStringKanji(0x60, static_cast<J2DTextBox*>(mpScreen->search(text_sv[i]))->getStringPtr(), NULL);
        dMeter2Info_getStringKanji(0x5F, static_cast<J2DTextBox*>(mpScreen->search(text_op[i]))->getStringPtr(), NULL);
        ...
        dMeter2Info_getStringKanji(0x60, static_cast<J2DTextBox*>(mpScreen->search(ftext_sv[i]))->getStringPtr(), NULL);
        dMeter2Info_getStringKanji(0x5F, static_cast<J2DTextBox*>(mpScreen->search(ftext_op[i]))->getStringPtr(), NULL);
    ...
    for (int i = 0; i < 5; i++)
    {
        static_cast<J2DTextBox*>(mpScreenIcon->search(text_a_tag[i]))->setFont(mDoExt_getMesgFont());
        static_cast<J2DTextBox*>(mpScreenIcon->search(text_b_tag[i]))->setFont(mDoExt_getMesgFont());
    ...
    field_0x22d[2][0] = 0;
    field_0x22d[3][0] =
        dComIfGs_isItemFirstBit(0x28) || (dComIfGs_isItemFirstBit(0x3F)
    ...
    field_0x22d[4][0] = dComIfGs_isItemFirstBit(0x29) || dComIfGs_isItemFirstBit(0x49) ? true : false;
    ...
    field_0x22d[3][1] = dComIfGs_isItemFirstBit(0x2B) || dComIfGs_isItemFirstBit(0x2A) ? true : false;
...
{
    OSInitFastCast();
    void* resource = JKRGetNameResource("zelda_collect_soubi_screen.btk", dComIfGp_getCollectResArchive());
...
void dMenu_Collect2D_c::cursorMove()
{
    ...
    if (mpStick->checkRightTrigger())
    {
        if (mCursorX < 6)
        {
            if (mCursorX == 0 && mCursorY == 0 && field_0x259 >= 3 && field_0x25a < 3)
            {
    ...
    else
    {
        ...
        {
            if (mCursorX == 6 && mCursorY == 0 && field_0x259 >= 3 && field_0x25a < 3)
            {
                mCursorX = field_0x259;
                mCursorY = field_0x25a;
            }
            else if (dVar1 == 5 && dVar2 == 0 && field_0x259 == 4 && field_0x25a == 1)
            {
                mCursorX = field_0x259;
                mCursorY = field_0x25a;
            }
            ...
            for (int j = 0; j < 6; j++)
            {
                if (getItemTag(i, j, true))
                {
                    ...
                    if ((i != 0 || j != 0) && (i != 6 || j != 0))
                    {
                        ...
                        {
                            pane->scale(g_drawHIO.mCollectScreen.mSelectItemScale, g_drawHIO.mCollectScreen.mSelectItemScale);
                        }
                        else
                        {
        ...
        for (int j = 0; j < 6; j++)
        {
            if (getItemTag(i, j, true) != 0)
            {
                ...
                if ((i != 0 || j != 0) && (i != 6 || j != 0))
                {
                    ...
                    {
                        itemTag->scale(g_drawHIO.mCollectScreen.mSelectItemScale, g_drawHIO.mCollectScreen.mSelectItemScale);
                    }
                    else
                    {
            ...
            for (int i = 0; i < 2; i++)
            {
                if (i == i_frame && field_0x22d[i + 3][0] != 0)
                {
                    static_cast<J2DPicture*>(mpScreen->search(tag[i]))
                        ->setBlackWhite(JUtility::TColor(0, 0, 0, 0), JUtility::TColor(255, 255, 0, 255));
                }
                else
                {
                    static_cast<J2DPicture*>(mpScreen->search(tag[i]))
                        ->setBlackWhite(JUtility::TColor(0, 0, 0, 0), JUtility::TColor(107, 107, 107, 255));
                }
            }
        }
    }
...
void dMenu_Collect2D_c::setHIO(bool i_useHIO)
{
    ...
    if (mBlueSmokePosX != g_drawHIO.mCollectScreen.mBlueSmokePosX || mBlueSmokePosY != g_drawHIO.mCollectScreen.mBlueSmokePosY)
    {
    ...
    if (g_drawHIO.mCollectScreen.mColorDebugON)
    {
        ...
        basePicture->setBlackWhite(g_drawHIO.mCollectScreen.mVesselBack[0], g_drawHIO.mCollectScreen.mVesselFront[0]);

        for (int i = 0; i < 9; i++)
        {
            J2DPicture* kazPicture = (J2DPicture*)mpScreen->search(kaz_n[i]);
            kazPicture->setBlackWhite(g_drawHIO.mCollectScreen.mVesselBack[1], g_drawHIO.mCollectScreen.mVesselFront[1]);
        }
    }
        ...
        else
        {
            if (field_0x22d[mCursorX][mCursorY] != 0 && !mIsWolf)
            {
                if ((mCursorX >= 3 && mCursorX <= 4) || (mCursorX == 5 && mCursorY == 2))
                {
                    ...
                    else if (cursorY == 2 && daPy_getPlayerActorClass()->getClothesChangeWaitTimer() == 0)
                    {
                        changeClothe();
                    }
                }
...
void dMenu_Collect2D_c::setAButtonString(u16 i_stringID)
{
    ...
    if (i_stringID != mCurrentAString)
    {
        ...
        if (i_stringID == 0)
        {
            for (int i = 0; i < 5; i++)
            {
                char* stringPtr = static_cast<J2DTextBox*>(mpScreenIcon->search(text_a_tag[i]))->getStringPtr();
                strcpy(stringPtr, "");
            }
        }
        else
...
void dMenu_Collect2D_c::setBButtonString(u16 i_stringID)
{
    ...
    if (i_stringID != mCurrentBString)
    {
        ...
        if (i_stringID == 0)
        {
            for (int i = 0; i < 5; i++)
            {
                char* stringPtr = static_cast<J2DTextBox*>(mpScreenIcon->search(text_b_tag[i]))->getStringPtr();
                strcpy(stringPtr, "");
            }
        }
        else
...
void dMenu_Collect2D_c::setItemNameString(u8 param_0, u8 param_1)
{
    ...
    else
    {
        ...
        {
#if REGION_JPN
            char* stringPtr = static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('item_n00')))->getStringPtr();
            ...
            dMeter2Info_getStringKanji(uVar6, stringPtr, NULL);
#else
            char* stringPtr = static_cast<J2DTextBox*>(mpScreen->search(MULTI_CHAR('item_n04')))->getStringPtr();
            dMeter2Info_getStringKanji(uVar6, stringPtr, NULL);
...
dMenu_Collect3D_c::dMenu_Collect3D_c(JKRExpHeap* param_0, dMenu_Collect2D_c* param_1, CSTControl* param_2)
{
    mIsWolf = daPy_py_c::checkNowWolf();
    mpHeap = param_0;
    mpSolidHeap = NULL;
...
void dMenu_Collect3D_c::_move(u8 param_0, u8 param_1)
{
    ...
    if (mpModel != NULL)
    {
        ...
        f32 offsetPosX = maskPos.x + g_drawHIO.mCollectScreen.mMaskMirrorPos.x + mMaskMirrorOffsetX + offsetX;
        f32 offsetPosY = maskPos.y + g_drawHIO.mCollectScreen.mMaskMirrorPos.y + mMaskMirrorOffsetY + offsetY;
...
void dMenu_Collect3D_c::setJ3D(const char* param_0, const char* param_1, const char* param_2)
{
    ...
    J3DModelData* modelData = J3DModelLoaderDataBase::load(resArchive->getResource('BMD ', param_0), 0x51020010);
    ...
    if (param_1 != NULL)
    {
        J3DAnmTransform* pbck =
            (J3DAnmTransform*)J3DAnmLoaderDataBase::load(resArchive->getResource('BCK ', param_1), J3DLOADER_UNK_FLAG0);
    ...
    if (param_2 != NULL)
    {
        J3DAnmTevRegKey* pbrk =
            (J3DAnmTevRegKey*)J3DAnmLoaderDataBase::load(resArchive->getResource('BRK ', param_2), J3DLOADER_UNK_FLAG0);
...
void dMenu_Collect2DTop_c::draw()
{
    ...
    graf_port->setup2D();
    mpCollect2D->getBlackTex()->draw(mDoGph_gInf_c::getMinXF(),
