...
void customMenuRing_c::setUpItemWheelMenuText() {
    ...
    // g_seedInfo.getHeaderPtr()->getSeedNamePtr()); strcat(itemWheelTextBuf, tempBuf);
    ...
    snprintf(itemWheelTextBuf, sizeof(itemWheelTextBuf),
             "Shadows: %d/3        Key Legend:\nShards: %d/4         Current "
             "(Total)\n\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n",
             shadowsCount, shardsCount, "Forest", "Mines", "Lakebed", "Arbiters", "Snowpeak",
             "Time", "City", "Palace", "Hyrule", "Desert", "Faron Gate", "Coro Gate", "Gate Keys");
    ...
    snprintf(itemWheelTextBuf, sizeof(itemWheelTextBuf),
             "\n\n\n%d (%d)\n%d (%d)\n%d (%d)\n%d (%d)\n%d (%d)\n%d (%d)\n%d (%d)\n%d (%d)\n%d "
             "(%d)\n%d (%d)\n%s\n%s\n%s\n",
             ftKeyNum, ftTotalKeyNum, gmKeyNum, gmTotalKeyNum, lbtKeyNum, lbtTotalKeyNum, agKeyNum,
             agTotalKeyNum, sprKeyNum, sprTotalKeyNum, totKeyNum, totTotalKeyNum, citsKeyNum,
             citsTotalKeyNum, potKeyNum, potTotalKeyNum, hcKeyNum, hcTotalKeyNum, campKeyNum,
             campTotalKeyNum, getYesNoText(hasFaronGateKey), getYesNoText(hasCoroGateKey),
             getYesNoText(hasGateKey));
...
void customMenuRing_c::handleItemWheelMenu(dMenu_Ring_c* dMenuRingPtr) {
    ...
    drawFilledRect(ringPosX + windowPosXOffset, ringPosY + windowPosYOffset, 305.f, 410.f,
                   colorBlk);
    ...
    for (int i = 0x10; i < 0x19; i++) {
        if (dComIfGs_isDungeonItemBossKey(i)) {
            if (i == 0x11) {
                g_randoInfo.gmKeyIconPtr->draw(ringPosX + windowPosXOffset + 170.f,
                                               ringPosY + windowPosYOffset, 23.f, 23.f, false,
                                               false, false);
            } else if (i == 0x14) {
                g_randoInfo.bedKeyIconPtr->draw(ringPosX + windowPosXOffset + 170.f,
                                                ringPosY + windowPosYOffset, 23.f, 23.f, false,
                                                false, false);
            } else {
                g_randoInfo.getBigKeyIconPtr()->draw(ringPosX + windowPosXOffset + 170.f,
                                                     ringPosY + windowPosYOffset, 23.f, 23.f, false,
                                                     false, false);
            }
        }
        ...
        if (dComIfGs_isDungeonItemMap(i)) {
            g_randoInfo.getDunMapIconPtr()->draw(ringPosX + windowPosXOffset + 195.f,
                                                 ringPosY + windowPosYOffset, 23.f, 23.f, false,
                                                 false, false);
        }
        if (dComIfGs_isDungeonItemCompass(i)) {
            g_randoInfo.getDunCompassIconPtr()->draw(ringPosX + windowPosXOffset + 220.f,
                                                     ringPosY + windowPosYOffset, 23.f, 23.f, false,
                                                     false, false);
        }
        ...
        if (i == 0x14) {
            if (dComIfGs_isEventBit(TOLD_YETA_ABOUT_PUMPKIN)) {
                g_randoInfo.pumpkinIconPtr->draw(ringPosX + windowPosXOffset + 245.f,
                                                 ringPosY + windowPosYOffset, 23.f, 23.f, false,
                                                 false, false);
            }
            if (dComIfGs_isEventBit(TOLD_YETA_ABOUT_CHEESE)) {
                g_randoInfo.cheeseIconPtr->draw(ringPosX + windowPosXOffset + 270.f,
                                                ringPosY + windowPosYOffset, 23.f, 23.f, false,
                                                false, false);
            }
        }
